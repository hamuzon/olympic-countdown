...
export async function onRequest(context) {
  ...
  let lang = (url.searchParams.get("lang") || "").trim().toLowerCase();
  const qCP =
  ...
  if (!year && qCP) {
    ...
    if (!lang && fromCreatePath.lang)
      lang = String(fromCreatePath.lang).trim().toLowerCase();
  }
  ...
  const cleanBasePath = `/${pathParts
    .slice(0, yearIdx === -1 ? pathParts.length : yearIdx)
    .filter((p) => p !== "ja" && p !== "en" && !/^\d{4}$/.test(p))
    .join("/")}`.replace(/\/+$|^$/, "/");

  const canonicalRedirectUrl = new URL(url.origin);
  if ((URL_SETTINGS.urlScheme || "path") === "query") {
    canonicalRedirectUrl.pathname = cleanBasePath;
    canonicalRedirectUrl.searchParams.set("year", year);
    canonicalRedirectUrl.searchParams.set("lang", lang);
  } else {
    canonicalRedirectUrl.pathname = `${cleanBasePath.replace(/\/$/, "")}/${year}/${lang}`;
  }
  ...
  if (ogUrlScheme === "query") {
    ...
    canonicalUrl.pathname = `${cleanBasePath.replace(/\/$/)}/${year}/${lang}`;
  }
  // --- HTML Rewriting ---
